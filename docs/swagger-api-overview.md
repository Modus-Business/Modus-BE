...
- RFR 구조의 토큰 회전 흐름에 해당합니다.

### `POST /auth/logout`
- 서버에 저장된 refresh token을 폐기하는 로그아웃 API입니다.

### `POST /auth/email/send-verification`
- 로그인한 사용자의 이메일로 인증 코드를 발송하는 API입니다.
- 인증 코드는 영문 대소문자 3개와 숫자 3개로 구성된 6자리입니다.

### `POST /auth/email/verify`
- 로그인한 사용자가 이메일 인증 코드를 검증하는 API입니다.
- 검증이 성공하면 `isEmailVerified` 값이 `true`로 변경됩니다.
...
- 같은 `classes` 경로를 쓰지만, 역할에 따라 응답 목적이 다릅니다.
...
### `POST /groups`
- 교강사 모둠 생성 API입니다.
- 특정 수업 안에서 모둠명과 학생 목록으로 모둠을 만듭니다.
- 모둠에 들어온 학생은 이 시점에 익명 닉네임이 자동 생성됩니다.

### `PATCH /groups/:groupId`
- 교강사 모둠 수정 API입니다.
- 모둠명과 학생 목록을 바꿉니다.
- 모둠에서 빠진 학생의 익명 닉네임은 삭제되고, 새로 들어온 학생은 새 익명 닉네임을 받습니다.

### `DELETE /groups/:groupId`
- 교강사 모둠 삭제 API입니다.
- 모둠 삭제 시 멤버와 익명 닉네임도 함께 정리됩니다.
...
### `GET /groups/:groupId`
- 모둠 상세 조회 API입니다.
- 교강사는 실명 기준 멤버 목록을 보고, 학생은 저장된 익명 닉네임 기준 멤버 목록을 봅니다.
