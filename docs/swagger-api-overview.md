...
현재 Swagger에 등록된 주요 API와 응답 구조를 빠르게 확인하기 위한 문서입니다.

## 공통 응답 구조

### 성공 응답
- 모든 성공 응답은 전역 응답 인터셉터를 통해 아래 형태로 감싸집니다.
...
### 에러 응답
- 모든 에러 응답은 전역 예외 필터를 통해 아래 형태로 반환됩니다.
...
- 사용자 생성 시점에 `isEmailVerified`는 `true`입니다.
...
### `GET /me/settings`
- 설정 화면에 필요한 현재 사용자 정보를 조회합니다.

## Classes

### `GET /classes`
- 메인 화면 수업 목록을 조회합니다.
- 학생은 참여 중인 수업과 `myGroup` 정보를 받고, 교사는 자신이 만든 수업과 수업 코드를 받습니다.

### `GET /classes/:classId/participants`
- 교사용 수업 참가 학생 목록 조회 API입니다.
...
### `GET /groups/teacher/:groupId`
- 교강사용 모둠 조회 API입니다.
- 실명 기준 멤버 목록을 반환합니다.

### `GET /groups/student/:groupId`
- 학생용 모둠 조회 API입니다.
- 익명 닉네임 기준 멤버 목록을 반환합니다.
...
- 프론트는 이 URL로 직접 업로드한 뒤, 반환된 `fileUrl`을 결과물 제출 API에 전달합니다.
